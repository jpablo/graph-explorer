...
object Svg:
  ...
  def svg(g: RGraph): String =
    ...
    // neither. (g.name == the graph id; None ⇒ anonymous.) An EXPLICITLY
    // empty name (`graph "" {`) is anonymous too — gv gates both on
    // `agnameof(g)[0] != '\0'` (svg_begin_graph / svg_begin_page).
    val gname = g.name.filter(_.nonEmpty)
    sb ++= (gname match
    ...
    def emitNode(i: Int): Unit =
      val n = g.nodes(i)
      // emit_node (emit.c:1873): any style token `invis` returns right after
      // the name comment — no <g>, no shape, no xlabel. Object ids are
      // seq-based, so later nodes are unaffected (switch: `a` stays node17).
      if n.attrs.get("style").iterator.flatMap(_.split(",")).map(_.trim).contains("invis") then
    ...
    def emitEdge(ix: Int, ei: Int): Unit =
      ...
      spl.get(ix).foreach { es => scala.util.boundary {
          ...
          // color: perpendicular offsets per control point (computeoffset_p
          // at segment joins, computeoffset_qr — shared by both interior
          // points — inside a segment; SEP=2, EPSILON=1e-4 stabilized),
          // starting at pt − (2+numc)/2·off and advancing +off per color
          // (strtok skips empty components; numc counts RAW ':'s). The tail
          // arrow takes the 2nd color, the head arrow the 1st.
          val numc  = eStroke.count(_ == ':')
          val comps = eStroke.split(":").iterator.filter(_.nonEmpty).toVector
          val multi = numc > 0 && !eStroke.contains(';')
          if multi then
            val SepE = 2.0; val EpsE = 0.0001
            val np = pts.length
            def offP(p: Spline.XY, q: Spline.XY): (Double, Double) =
              val x = p.x - q.x; val y = p.y - q.y
              val d = SepE / math.sqrt(x * x + y * y + EpsE)
              (y * d, -x * d)
            def offQR(p: Spline.XY, q: Spline.XY, r: Spline.XY, s: Spline.XY): (Double, Double) =
              var x = q.x - r.x; var y = q.y - r.y
              var len = math.hypot(x, y)
              if len < EpsE then
                x = p.x - s.x; y = p.y - s.y
                len = math.sqrt(x * x + y * y + EpsE)
              val d = SepE / len
              (y * d, -x * d)
            val off = new Array[(Double, Double)](np)
            var pf3 = pts(0); var pf2 = Spline.XY(0, 0)
            var j = 0
            while j < np - 1 do
              val pf0 = pf3; val pf1 = pts(j + 1)
              off(j) = if j == 0 then offP(pf0, pf1) else offP(pf2, pf1)
              pf2 = pts(j + 2); pf3 = pts(j + 3)
              val o = offQR(pf0, pf1, pf2, pf3)
              off(j + 1) = o; off(j + 2) = o
              j += 3
            off(j) = offP(pf2, pf3)
            val numc2 = (2.0 + numc) / 2.0
            val tmp = Array.tabulate(np)(k => (pts(k).x - numc2 * off(k)._1, pts(k).y - numc2 * off(k)._2))
            comps.foreach { c0 =>
              var k = 0
              while k < np do
