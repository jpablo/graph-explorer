...
| INV-08 | Baseline end-to-end internal phases behavior remains stable | `InternalPhasesSpec` sanity and DOT roundtrip checks (`viewer/src/test/scala/org/jpablo/graphexplorer/viewer/state/InternalPhasesSpec.scala:23`) | pass | Re-verified after fold-only UI dispatch and explicit port additions in latest three-suite run |
