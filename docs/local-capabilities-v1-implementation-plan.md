...
| LC2-T2 | WS2 | Implement filesystem watcher with debounce and coalescing | ✅ | LC2-T1 | External file change emits one normalized update | Polling watcher loop added (15ms poll, 50ms debounce/coalescing by content hash; tuned from 100/120 -> 30/75 -> 15/50 for the LC2-T5 latency budget on slow CI runners) with revision increment on emitted update | Uses per-watch background thread and stop channel |
