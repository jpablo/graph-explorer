...
| **Node fill/border/font attrs (`fillcolor`, `color`, `font*`, `penwidth`)**<br>`D->GE`: mostly direct attr mapping.<br>`GE->D`: mostly direct attr emission. | `M->GE`: class/default/inline Mermaid styles are resolved to effective toolbar attrs in read path.<br>`GE->M`: full write-back from normalized attrs is still in progress (`MP3` track).<br>`gap`: remaining write-side parity. | DOT-like attrs (`FillColor`, `Color`, `FontColor`, `FontName`, `FontSize`, `PenWidth`, etc.). |
