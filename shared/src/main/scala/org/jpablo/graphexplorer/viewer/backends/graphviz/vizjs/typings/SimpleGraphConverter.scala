...
//      if (!exclude.contains("_subgraph_cnt")) attrs += AttributeId("_subgraph_cnt") -> AttrValue(graph._subgraph_cnt.toString)
      ...
      graph.labelloc.foreach(v => if (!exclude.contains("labelloc")) attrs += AttributeId("labelloc") -> AttrValue(v))
      graph.lp.foreach(v => if (!exclude.contains("lp")) attrs += AttributeId("lp") -> AttrValue(v))
      graph.lheight.foreach(v => if (!exclude.contains("lheight")) attrs += AttributeId("lheight") -> AttrValue(v))
      graph.lwidth.foreach(v => if (!exclude.contains("lwidth")) attrs += AttributeId("lwidth") -> AttrValue(v))
      graph.rankdir.foreach(v => if (!exclude.contains("rankdir")) attrs += AttributeId("rankdir") -> AttrValue(v))
      graph.layout.foreach(v => if (!exclude.contains("layout")) attrs += AttributeId("layout") -> AttrValue(v))
      graph.bgcolor.foreach(v => if (!exclude.contains("bgcolor")) attrs += AttributeId("bgcolor") -> AttrValue(v))
      graph.nodesep.foreach(v => if (!exclude.contains("nodesep")) attrs += AttributeId("nodesep") -> AttrValue(v))
      graph.pad.foreach(v => if (!exclude.contains("pad")) attrs += AttributeId("pad") -> AttrValue(v))
      graph.ranksep.foreach(v => if (!exclude.contains("ranksep")) attrs += AttributeId("ranksep") -> AttrValue(v))
      graph.ratio.foreach(v => if (!exclude.contains("ratio")) attrs += AttributeId("ratio") -> AttrValue(v))
      graph.splines.foreach(v => if (!exclude.contains("splines")) attrs += AttributeId("splines") -> AttrValue(v))
      graph.overlap.foreach(v => if (!exclude.contains("overlap")) attrs += AttributeId("overlap") -> AttrValue(v))
      graph.normalize.foreach(v => if (!exclude.contains("normalize")) attrs += AttributeId("normalize") -> AttrValue(v))
      graph.start.foreach(v => if (!exclude.contains("start")) attrs += AttributeId("start") -> AttrValue(v))
...
//        _subgraph_cnt = subgraphCnt,
  ...
  def graphToDotString(graph: SimpleGraph, omitInternal: Boolean = false): String =
    ...
    def collectEdgeAttributes(edge: SimpleGraphEdge, excludeKeys: Set[String] = Set.empty): List[(String, String)] =
      ...
      edge.id.foreach(v => if (!allExcludeKeys.contains("id")) attrs += "id" -> v)
      edge.label.foreach(v => if (!allExcludeKeys.contains("label")) attrs += "label" -> v)
      edge.fontname.foreach(v => if (!allExcludeKeys.contains("fontname")) attrs += "fontname" -> v)
      edge.fontsize.foreach(v => if (!allExcludeKeys.contains("fontsize")) attrs += "fontsize" -> v)
      edge.color.foreach(v => if (!allExcludeKeys.contains("color")) attrs += "color" -> v)
      edge.penwidth.foreach(v => if (!allExcludeKeys.contains("penwidth")) attrs += "penwidth" -> v)
      edge.style.foreach(v => if (!allExcludeKeys.contains("style")) attrs += "style" -> v)
      edge.lp.foreach(v => if (!allExcludeKeys.contains("lp")) attrs += "lp" -> v)
      edge.len.foreach(v => if (!allExcludeKeys.contains("len")) attrs += "len" -> v)
      edge.constraint.foreach(v => if (!allExcludeKeys.contains("constraint")) attrs += "constraint" -> v)
      edge.forcelabels.foreach(v => if (!allExcludeKeys.contains("forcelabels")) attrs += "forcelabels" -> v)
      edge.arrowhead.foreach(v => if (!allExcludeKeys.contains("arrowhead")) attrs += "arrowhead" -> v)
      edge.arrowtail.foreach(v => if (!allExcludeKeys.contains("arrowtail")) attrs += "arrowtail" -> v)
      edge.arrowsize.foreach(v => if (!allExcludeKeys.contains("arrowsize")) attrs += "arrowsize" -> v)
      edge.dir.foreach(v => if (!allExcludeKeys.contains("dir")) attrs += "dir" -> v)
      edge.`class`.foreach(v => if (!allExcludeKeys.contains("class")) attrs += "class" -> v)
      edge.colorscheme.foreach(v => if (!allExcludeKeys.contains("colorscheme")) attrs += "colorscheme" -> v)
      edge.layer.foreach(v => if (!allExcludeKeys.contains("layer")) attrs += "layer" -> v)
      edge.nojustify.foreach(v => if (!allExcludeKeys.contains("nojustify")) attrs += "nojustify" -> v)
      edge.samehead.foreach(v => if (!allExcludeKeys.contains("samehead")) attrs += "samehead" -> v)
      edge.sametail.foreach(v => if (!allExcludeKeys.contains("sametail")) attrs += "sametail" -> v)
      edge.showboxes.foreach(v => if (!allExcludeKeys.contains("showboxes")) attrs += "showboxes" -> v)
      edge.tail_lp.foreach(v => if (!allExcludeKeys.contains("tail_lp")) attrs += "tail_lp" -> v)
      edge.tailclip.foreach(v => if (!allExcludeKeys.contains("tailclip")) attrs += "tailclip" -> v)
      edge.target.foreach(v => if (!allExcludeKeys.contains("target")) attrs += "target" -> v)
      edge.tooltip.foreach(v => if (!allExcludeKeys.contains("tooltip")) attrs += "tooltip" -> v)
      edge.labeldistance.foreach(v => if (!allExcludeKeys.contains("labeldistance")) attrs += "labeldistance" -> v)
      edge.labelfloat.foreach(v => if (!allExcludeKeys.contains("labelfloat")) attrs += "labelfloat" -> v)
      edge.labelfontcolor.foreach(v => if (!allExcludeKeys.contains("labelfontcolor")) attrs += "labelfontcolor" -> v)
      edge.labelfontname.foreach(v => if (!allExcludeKeys.contains("labelfontname")) attrs += "labelfontname" -> v)
      edge.tailtarget.foreach(v => if (!allExcludeKeys.contains("tailtarget")) attrs += "tailtarget" -> v)
      edge.tailtooltip.foreach(v => if (!allExcludeKeys.contains("tailtooltip")) attrs += "tailtooltip" -> v)
      edge.tailURL.foreach(v => if (!allExcludeKeys.contains("tailURL")) attrs += "tailURL" -> v)
      attrs.toList
