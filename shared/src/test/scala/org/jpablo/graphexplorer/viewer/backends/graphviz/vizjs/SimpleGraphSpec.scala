...
import org.jpablo.graphexplorer.viewer.backends.graphviz.vizjs.simplegraph.{ArrowPositionParser, Point, SimpleGraph}
...
class SimpleGraphSpec extends FunSuite:
  ...
  test("getEdgePos should extract edge positions from graph JSON with three arrow variants") {
    ...
    val edgePositions = simplegraph.getEdgePos(graph)
  ...
  test("getEdgePos should handle empty edges") {
    ...
    val edgePositions = simplegraph.getEdgePos(emptyGraph)
