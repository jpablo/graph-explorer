...
| M6 | Long tail: clusters, ports, compass, `rankdir`, record & HTML-like labels | Per-feature rows in §5 all ✅ | 🟡 in progress (118/118) — ✅ done: records (layout+svg), **ports/compass**, **self-loops/parallel edges**, edge-label rank-doubling+Y, svg graph/edge titles, bbox precision, arrow miter + true `arrow_length` (directed corpus byte-exact), **03 clusters structural** (subgraph-tree model + `dot_json` byte-exact — §5.2). 🟡 `rankdir`: blocker 1 (layout-size) ✅, blocker 2 half (weight ✅, label-vnode-X-under-flip open). ⬜ remaining: `rankdir` label-vnode-X, `rank=same` *layout* constraint, `json0`/svg cluster geometry, HTML-like labels. M6 stays ⬜-at-milestone-level until *all* §5 rows ✅; per-row truth = §5, narrative = §7 |
...
| Feature | Status | Test | Notes |
|---|---|---|---|
| Poly shapes: box, ellipse, circle, square, plaintext, none, plain | 🟡 | NodeSizeSpec + StyleSvgSpec | sized & oracle-verified; **box/rect/rectangle/square render as a rectangle `<polygon>` + use `poly_inside` box edge-clipping — `10-box` svg fully byte-exact (2026-05-29)**; ellipse/circle = `<ellipse>`. Also fixed: svg `id="edgeN"` = AGSEQ (declaration) index, decoupled from emit order (was latent). Deferred: `rounded` box path (bezier corners); diamond/polygon/point |
