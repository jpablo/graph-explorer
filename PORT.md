...
- **2026-07-12** — **Cluster-aware mincross — single-level contiguity byte-exact
  ...
  deferred (fails-when-fixed): a cluster spanning multiple ranks with FREE long
  edges crossing into it needs gv's **remincross** pass + `left2right` to order
  the free virtuals against the skeleton column (~10pt x-shift until then).
  Nested clusters also TODO (recurse the collapse per level). Corpus **89/95**
  byte-exact; graphvizJVM 520/520, JS green, gv worktree pristine.
