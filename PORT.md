...
| Feature | Status | Test | Notes |
|---|---|---|---|
| Poly shapes: box, ellipse, circle, square, plaintext, none, plain | 🟡 | NodeSizeSpec + StyleSvgSpec | sized & oracle-verified; **box/rect/rectangle/square render as a rectangle `<polygon>` in svg (2026-05-29, `10-box` node `<g>` byte-exact)**; ellipse/circle = `<ellipse>`. Deferred: edge **box-clipping** (`poly_inside` vs `ellipse_inside` in `clip_and_install` — 10-box edge splines still ellipse-clipped); `rounded` box path; diamond/polygon/point |
