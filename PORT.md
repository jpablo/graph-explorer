...
| M6 | Long tail: clusters, ports, compass, `rankdir`, record & HTML-like labels | Per-feature rows in §5 all ✅ | 🟡 in progress (114/114) — ✅ done: records (layout+svg), **ports/compass**, **self-loops/parallel edges**, edge-label rank-doubling+Y, svg graph/edge titles, bbox precision, arrow miter + true `arrow_length` (directed corpus byte-exact). 🟡 `rankdir`: blocker 1 (layout-size) ✅, blocker 2 half (weight ✅, label-vnode-X-under-flip open). ⬜ remaining: **03 clusters** (subgraph-tree model + instrument-derived ownership rule — §5.2 cluster row), `rank=same`, HTML-like labels. M6 stays ⬜-at-milestone-level until *all* §5 rows ✅; per-row truth = §5, narrative = §7 |
...
| Cluster layout (recursive) | ⬜ (recon: **reframed — geometry-free oracle**) | | **Recon 2026-05-17 (major de-risk):** viz-js 3.14.0 (the pinned oracle) does **not lay out clusters** — 03's `plain`/`dot`/`json0` are all `pos="0,0"`, `bb="0,0,0,0"`. So 03 is **not a recursive-layout port** (the "hardest part of M6" framing was wrong); per §1 ("match viz-js *strings*") it's a **structural model+emit** task with NO geometry. Exact target from the golden: `_subgraph_cnt=3`; `objects` = the 3 subgraph objects **first** (`cluster_0`,`cluster_1`, anon `{rank=same}`→`%7`) then the 6 nodes; each subgraph object carries `name`/`label`/`nodes=[gvid…]`/`edges=[gvid…]`; node `_gvid` is offset by `_subgraph_cnt` (a0=3…start=8); edges `_gvid` by cgraph order. **Model-level blocker** (the real work): `AttrResolver`/`RGraph` is flat (no subgraph tree) — needs an additive cluster/subgraph tree threaded into `Output` (safe: non-clustered corpus has `_subgraph_cnt=0` ⇒ unchanged). **Sharpened 2026-05-17** (read `plugin/core/gvrender_core_json.c`): `_gvid` rule is clear — `label_subgs` preorder-DFS over `agfstsubg` (0-based, root excluded) = `_subgraph_cnt`, then nodes `sgcnt + ncnt++` in root decl order. **The genuine unknown is the node→subgraph *ownership* partition**: `a0` ∈ `cluster_0` (edge `a0→a1` there) **and** `%7` (`{rank=same; a0}`) but the golden lists it **only under `%7`** (`cluster_0.nodes=[a1,a2]`). `write_subg` walks `agfstnode(sg)` with no dedup flag, so the partition is whatever cgraph's per-subgraph node membership is *after dot's cluster processing* (node induction / cluster_leader) — **not inferable from one example or source-reading; needs an instrumented-gv `agfstnode(sg)` dump (§2.5)**. So 03 = additive subgraph-tree model **+** an instrument-derived ownership rule: a focused *dedicated* session, not a tail-of-session commit (faking the rule = fake green) |
