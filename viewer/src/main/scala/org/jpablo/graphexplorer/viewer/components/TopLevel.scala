...
import org.jpablo.graphexplorer.viewer.formats.dot.ast.GraphElementAttributes
...
def TopLevel(state: ViewerState, router: Router): ReactiveHtmlElement[HTMLDivElement] =
  ...
  val attrs = Var(GraphElementAttributes())
  div(
    idAttr := "top-level",
    child(GraphAttributesView(state)) <-- state.diagramAttributesVisible.signal,
