    ...
    .amend {
      Seq(
        ...
        onMouseDown.filter(leftButton).map(clientCoords) --> { (pos, shift) =>
          val rect          = MouseActionRect(pos, pos, shift)
          val selectionRect = viewerOps.DrawSelectionRect(mainGroup, rect).ref
          mouseAction.start(ExtendSelectionAction(rect, selectionRect))
        },
        ...
        child.maybe <--
          mouseAction.signal.map:
            case a: ExtendSelectionAction => Some(viewerOps.DrawSelectionRect(mainGroup, a.rect))
            case _                        => None,
        ...
        // --------------------------------------------------------
        selectionGroups --> { (toUnselect, toSelect) =>
