...
case class Arrow(source: NodeId, target: NodeId, idAttr: Option[String]):

  // Re-create the string used by graphviz in the `<title>` element of the SVG.
  def nodeId: NodeId = NodeId(s"${source.value}->${target.value}" + idAttr.map(i => s":$i").getOrElse(""))
...
object Arrow:
  ...
  def apply(t: (String, String), idx: Option[String]): Arrow = new Arrow(NodeId(t._1), NodeId(t._2), idx)
  ...
  def fromGraphvizTitle(title: String, idAttr: String): Option[Arrow] =
    ...
    if i > 0 && i < title.length - 2 then
      ...
      if l.nonEmpty && r.nonEmpty then
        Some(Arrow(NodeId(l), NodeId(r), Some(idAttr)))
      else
        None
    else
      None
  ...
  given scala.Ordering[Arrow] with
    def compare(x: Arrow, y: Arrow): Int =
      ...
      if s == 0 then
        ...
        if t == 0 then
          x.idAttr.getOrElse("").compareTo(y.idAttr.getOrElse(""))
        else
          t
      else
        s
