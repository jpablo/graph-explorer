...
class MermaidBackendNormalizationSpec extends FunSuite:
  ...
  test("jsMapToDict should convert a JS Map to a plain Dictionary") {
    // Mermaid v11 returns Map objects from getVertices()/getClasses()
    val jsMap = js.Dynamic.newInstance(js.Dynamic.global.Map)()
    jsMap.set("A", js.Dynamic.literal(id = "A", text = "CodeMirror"))
    jsMap.set("B", js.Dynamic.literal(id = "B", text = "Parser"))

    // Verify: Object.keys on a Map returns empty
    val objectKeys = js.Dynamic.global.Object.keys(jsMap).asInstanceOf[js.Array[String]]
    assertEquals(objectKeys.length, 0, "Object.keys on a Map should be empty (this is the bug)")

    // jsMapToDict should convert it to a usable Dictionary
    val dict = MermaidBackend.jsMapToDict(jsMap.asInstanceOf[js.Dictionary[js.Dynamic]])
    val keys = js.Object.keys(dict.asInstanceOf[js.Object])
    assertEquals(keys.length, 2, "Converted dictionary should have 2 entries")
    assertEquals(dict("A").text.asInstanceOf[String], "CodeMirror")
    assertEquals(dict("B").text.asInstanceOf[String], "Parser")
  }

  test("jsMapToDict should pass through a plain object unchanged") {
    val plainObj = js.Dictionary[js.Dynamic](
      "X" -> js.Dynamic.literal(id = "X", text = "Hello")
    )

    val result = MermaidBackend.jsMapToDict(plainObj)
    assertEquals(result("X").id.asInstanceOf[String], "X")
    assertEquals(result("X").text.asInstanceOf[String], "Hello")
  }
