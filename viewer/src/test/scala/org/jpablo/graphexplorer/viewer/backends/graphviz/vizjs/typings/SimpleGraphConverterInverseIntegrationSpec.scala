...
class SimpleGraphConverterInverseIntegrationSpec extends FunSuite with TestHelpers:
  ...
  test("fromViewerGraphElements should handle groups as clusters") {
    ...
    assert(cluster.label == "Group 1" || cluster.label == "cluster1", s"Cluster label should be 'Group 1' or 'cluster1', but was '${cluster.label}'")
    assertEquals(cluster.nodes.length, 2, "Cluster should contain 2 nodes")
  ...
  test("fromViewerGraphElements should handle nested groups as flattened clusters") {
    ...
    assert(parentCluster.label == "Parent Group" || parentCluster.label == "parent", 
    ...
    assert(childCluster.label == "Child Group" || childCluster.label == "child",
