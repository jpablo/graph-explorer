...
class ViewerStateSpec extends FunSuite with TestHelpers:
  ...
  test("DOT svgWithPositions updates after attribute change via toolbar path"):
    withGraphvizAsync { graphviz =>
      ...
      afterMicrotasks {
        ...
        // Change fill color via the toolbar path (zoomViaFullGraphUpdate)
  ...
  test("mermaid elementAttributesUpdates zoomed Var triggers sourceTextS update"):
    withGraphvizAsync { graphviz =>
      ...
      // Track sourceTextS emissions to verify the SVG pipeline would fire
      import com.raquo.laminar.api.L.unsafeWindowOwner
      given com.raquo.airstream.ownership.Owner = unsafeWindowOwner
      val sourceTextEmissions = Var(List.empty[String])
      viewerState.sourceTextS.foreach: text =>
      ...
      for
        ...
        _ <- afterMicrotasks {
          val serialized = viewerState.sourceTextNow()
          assert(serialized.contains("fill:#fb2c36"), s"Source text should include fill edit, got: $serialized")

          // Verify sourceTextS actually fired with the updated text.
          ...
          assert(
            ...
            s"sourceTextS should have emitted the updated text (needed for SVG re-render). Emissions: ${emissions.map(_.take(60))}"
          )
        }
      yield ()
    }
  ...
  test("mermaid svgWithPositions fires after attribute change via toolbar path"):
    withGraphvizAsync { graphviz =>
      ...
      for
        ...
        // Wait for the Mermaid render attempt (even if it fails in jsdom)
        _ <- waitForCondition(
  ...
  test("mermaid elementAttributesUpdates: source text remains stable after re-parse settles"):
    withGraphvizAsync { graphviz =>
      ...
      for
        ...
        _ <- afterMicrotasks {
          val textAfterEdit = viewerState.sourceTextNow()
        ...
        _ <- afterMicrotasks {
          val textAfterSettle = viewerState.sourceTextNow()
          assert(
